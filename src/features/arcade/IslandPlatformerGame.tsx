...
import { IslandPlatformerScene } from '@/game/scenes/IslandPlatformerScene';
import type { SkinId } from '@/game/kit/skins';
import type { PlatformerWorldId } from '@/game/data/levels/island';

type Props = {
  worldId: PlatformerWorldId;
  skin: SkinId;
  onResult: (r: { score: number; stars: number; cleared: boolean }) => void;
};
type Hud = {
...
const INITIAL_HUD: Hud = { hp: 100, maxHp: 100, lives: 3, score: 0, stars: 0, coins: 0, weapon: 'Water Jet', ammo: -1 };

export function IslandPlatformerGame({ worldId, skin, onResult }: Props) {
  const scenes = useRef([BootScene, IslandPlatformerScene]).current;
  const registry = useRef({
    skin, worldId,
    manifests: worldId === 'kilauea' ? ['kilauea'] : [],
    startScene: 'IslandPlat',
  }).current;
  ...
  const backToMap = useCallback(() => {
    onResultRef.current({ score: hud.score, stars: hud.stars, cleared: false });
  }, [hud]);
    ...
    <div className="relative w-full h-full select-none touch-none">
      ...
      {gameOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-center" style={{ background: 'rgba(20,8,4,0.86)' }}>
          ...
          <p className="text-sm" style={{ color: 'var(--color-cloud)' }}>The island got the better of you!</p>
          <Button onClick={tryAgain}>Try Again</Button>
          <button onClick={backToMap} className="tap-target text-xs underline" style={{ color: 'var(--color-cloud)', opacity: 0.8 }}>Back to map</button>
        </div>
      )}
    </div>
  );
